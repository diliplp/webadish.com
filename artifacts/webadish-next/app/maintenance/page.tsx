...
import { ArrowRight, BarChart3, CheckCircle2, ChevronRight, Clock, HeadphonesIcon, RefreshCw, Shield, Wrench } from 'lucide-react';
import SiteLayout from '@/components/SiteLayout';
import { Button } from '@/components/Button';

export const metadata: Metadata = {
  title: 'WordPress Maintenance Services | WebAdish',
  description: 'Safe updates, backups, monitoring, and hardening packaged as a premium WordPress maintenance service for business-critical websites.',
};
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link href="/pricing">
                <Button variant="outline" size="lg" className="text-lg px-8">See All Plans</Button>
              </Link>
            </div>
            <div className="flex flex-wrap gap-6 mt-10 text-sm text-muted-foreground font-medium">
              {['800+ sites protected', 'No lock-in contracts', '99.9% uptime guarantee', '20+ years in business'].map((badge) => (
                <div key={badge} className="flex items-center gap-2"><CheckCircle2 size={16} className="text-green-500" />{badge}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div key={plan.name} className={`rounded-3xl p-10 ${plan.highlight ? 'bg-accent text-white shadow-2xl shadow-accent/20 scale-105' : 'bg-white border border-border/50 shadow-sm'}`}>
                ...
                <ul className="space-y-3 mb-10">
                  {plan.features.map((feature) => (
                    <li key={feature} className={`flex items-center gap-3 text-sm ${plan.highlight ? 'text-white' : ''}`}>
                      <CheckCircle2 size={16} className={plan.highlight ? 'text-white shrink-0' : 'text-accent shrink-0'} />
                      {feature}
                    </li>
                  ))}
                </ul>
                <Link href="/contact">
                  <Button variant={plan.highlight ? 'white' : 'outline'} className="w-full">{plan.cta}</Button>
                </Link>
              </div>
            ))}
          </div>
          <div className="mt-10 text-center">
            <Link href="/india" className="text-sm font-medium text-accent hover:underline">
              Indian business pricing is available on the dedicated India page <ChevronRight size={16} className="inline-block" />
            </Link>
          </div>
        </div>
      </section>
