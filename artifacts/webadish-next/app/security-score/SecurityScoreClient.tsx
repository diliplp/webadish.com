...
import { AlertTriangle, ArrowRight, CheckCircle2, Gauge, LockKeyhole, Shield, Siren } from 'lucide-react';
...
export default function SecurityScoreClient() {
          ...
          <div className="max-w-4xl mx-auto bg-white rounded-3xl border border-border/50 p-6 md:p-8 shadow-sm text-left">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="md:col-span-2">
                ...
                <input type="text" placeholder="example.com" value={answers.url} onChange={(e) => update('url', e.target.value)} className="w-full rounded-xl border border-border bg-gray-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-3">What kind of site is this?</label>
                <select value={answers.siteType} onChange={(e) => update('siteType', e.target.value as Answers['siteType'])} className="w-full rounded-xl border border-border bg-gray-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent">
              ...
              <div>
                ...
                <select value={answers.plugins} onChange={(e) => update('plugins', e.target.value as Answers['plugins'])} className="w-full rounded-xl border border-border bg-gray-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent">
              ...
              <div>
                ...
                <select value={answers.lastAudit} onChange={(e) => update('lastAudit', e.target.value as Answers['lastAudit'])} className="w-full rounded-xl border border-border bg-gray-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent">
              ...
              <div>
                ...
                <select value={answers.backups} onChange={(e) => update('backups', e.target.value as Answers['backups'])} className="w-full rounded-xl border border-border bg-gray-50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent">
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  {[
                    { value: 'strong', title: 'Structured protection', text: 'Regular reviews, strong access control, tested backups, and a clear response plan.' },
                    { value: 'partial', title: 'Some basics in place', text: 'Updates and backups happen, but monitoring, audits, or response depth are inconsistent.' },
                    { value: 'weak', title: 'Mostly reactive', text: 'The site is maintained when something breaks or when someone remembers to check it.' },
                  ].map((item) => (
                    <button key={item.value} type="button" onClick={() => update('protection', item.value as Answers['protection'])} className={`rounded-2xl border p-4 text-left transition-all ${answers.protection === item.value ? 'border-accent bg-accent/5' : 'border-border/60 bg-gray-50 hover:border-accent/40'}`}>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <button type="button" onClick={() => setHasRun(true)} className="inline-flex items-center justify-center gap-2 rounded-xl px-8 py-4 text-base font-medium bg-accent text-accent-foreground shadow-lg shadow-accent/25 hover:bg-accent/90 hover:-translate-y-0.5 transition-all duration-300">
                Run Security Score <ArrowRight size={16} className="ml-1" />
              </button>
              <Link href="/contact" className="inline-flex items-center justify-center gap-2 rounded-xl px-8 py-4 text-base font-medium border-2 border-input bg-background hover:bg-muted hover:text-foreground transition-all duration-300">Request Full Security Review</Link>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="rounded-3xl border border-border/50 bg-gray-50 p-8 md:p-10">
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-8">
                ...
                <div className={`rounded-full px-5 py-3 text-sm font-semibold ${band.tone}`}>{band.label}</div>
              </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-6 items-start">
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      <Link href="/contact" className="inline-flex items-center justify-center gap-2 rounded-xl px-8 py-4 text-base font-medium bg-accent text-accent-foreground shadow-lg shadow-accent/25 hover:bg-accent/90 hover:-translate-y-0.5 transition-all duration-300">
                        Request Full Security Review <ArrowRight size={18} className="ml-1" />
                      </Link>
                      <Link href="/retainer" className="inline-flex items-center justify-center gap-2 rounded-xl px-8 py-4 text-base font-medium border border-white/20 text-white hover:bg-white/10 transition-all duration-300">
                        Explore Retainer
                      </Link>
                    </div>
                  </div>
